...
const fastify_1 = __importDefault(require("fastify")); // import the Fastify framework
const cors_1 = __importDefault(require("@fastify/cors")); // import the CORS plugin
const client_1 = require("./generated/prisma/client"); // import Prisma's database client
const adapter_pg_1 = require("@prisma/adapter-pg"); // adapter that connects Prisma to PostgreSQL
const bcrypt_1 = __importDefault(require("bcrypt")); // for hashing passwords
const jwt_1 = __importDefault(require("@fastify/jwt")); // for creating/verifying login tokens
const app = (0, fastify_1.default)({ logger: true }); // create the server, logger:true prints request logs
// create the adapter using our database connection string from .env
const adapter = new adapter_pg_1.PrismaPg({ connectionString: process.env.DATABASE_URL });
const prisma = new client_1.PrismaClient({ adapter }); // create a connection to our database
...
app.post("/employees", async (request, reply) => {
    const { name, email, phone, department, status } = request.body;
...
app.post("/departments", async (request, reply) => {
    const { name, manager, employeeCount } = request.body;
